SNF_margie <- function(Wall, K=20, t=20) {
  ...
  for (i in 1:t) {
    ...
    wall_iter[[i]] <- Wall #Margie code
  }
  names(wall_iter) <- c(1:t) #Margie code
  ...
  new_return <- list(result = W, knn_iterations = wall_iter) #Margie code
  return(new_return)  
}
